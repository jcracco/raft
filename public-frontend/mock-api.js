...
const DEMO_DEFAULTS = {
    project: {
        ...
        team_link: null,
        pointed_sp: 120,
        estimated_additional_sp: 0,
        buffer_pct: 20,
        avg_velocity: 30,
        velocity_auto: 0,
        current_weather_pct: 50,
        weather_auto: 0,
...
function loadDemoState() {
    try {
        ...
        return saved ? JSON.parse(saved) : JSON.parse(JSON.stringify(DEMO_DEFAULTS));
    } catch (e) {
...
function saveDemoState(state) {
    try {
        sessionStorage.setItem('raft_demo', JSON.stringify(state));
    } catch (e) {}
}
...
// Build done table
function buildDoneTable(entries, totalPts) {
    ...
    return entries.map(e => {
        ...
        return { ...e, focus_pct, points_remaining: Math.max(0, Math.round(remaining)) };
    });
}
...
function buildForecastTable(project, remaining, nextNumber, nextYear) {
    ...
    let badRem     = remaining;
    let currentRem = remaining;
    let goodRem    = remaining;
...
function buildProjectResponse(state) {
    ...
    const doneTable = buildDoneTable(entries, totalPts);
...
window._mockApi = async function(action, params = {}) {
    ...
    if (action === 'get_projects') {
        ...
        const doneTable    = buildDoneTable(state.entries, totalPts);
        const lastRemaining = doneTable.length > 0 ? doneTable[doneTable.length - 1].points_remaining : totalPts;
    ...
    if (action === 'update_project') {
        // Allow updating velocity and weather pct in demo
        const allowed  = ['initiative_name', 'pointed_sp', 'buffer_pct', 'avg_velocity', 'velocity_auto', 'current_weather_pct', 'weather_auto'];
        const numeric  = new Set(['pointed_sp', 'buffer_pct', 'avg_velocity', 'velocity_auto', 'current_weather_pct', 'weather_auto']);
        allowed.forEach(k => {
    ...
    // All other write actions blocked in demo
